...
def CPrint(msg_type, msg_string):
    if msg_type == 'HELP_MSG':
        color = txtcolors.HELP_MSG
    if msg_type == 'SUCCESS_MSG':
        color = txtcolors.SUCCESS_MSG
    if msg_type == 'CONTENT':
        color = txtcolors.CONTENT
    if msg_type == 'ERR_MSG':
        color = txtcolors.ERR_MSG
    if msg_type == 'NO_COLOR' or msg_type == '':
        color = txtcolors.NO_COLOR
    print(color, msg_string, txtcolors.NO_COLOR)

#
# Get list of files involved in this branch
target_files_data = subprocess.check_output(['git', 'diff', '--name-only', 'master'])
...
def VerifyClangFormatSource():
    ...
    retval = 0
    if not os.path.isfile('./scripts/clang-format-diff.py'):
        CPrint('HELP_MSG', "Missing /scripts/clang-format-diff.py, cannot run clang-format checks.")
        CPrint('HELP_MSG', "The missing script can be installed by running this command in the root of the repository:\n")
        CPrint('HELP_MSG', "    curl -L http://llvm.org/svn/llvm-project/cfe/trunk/tools/clang-format/clang-format-diff.py -o scripts/clang-format-diff.py")
    else:
        ...
        retval = 0
        git_diff = subprocess.Popen(('git', 'diff', '-U0', 'master', '--', diff_files), stdout=subprocess.PIPE)
...
def VerifyCopyrights():
    ...
    for file in target_files:
        if file is None or not os.path.isfile(file):
            continue
        for line in open(file, encoding="utf-8", errors='ignore').readlines():
            current_line += 1
            if current_line > 30:
                break;
            copyright_line = re.search('Copyright (.)*LunarG', line)
            if copyright_line:
                copyright_line = copyright_line.group(0)
                if not copyright_line.find(str(current_year)):
...
def VerifyCommitMessageFormat():
    ...
    if 'TRAVIS_BUILD_DIR' in os.environ:
        ...
        pr_commit_range_parms = ['git', 'log', '--no-merges', '--left-only', 'HEAD...master', '--pretty=format:"XXXNEWLINEXXX"%n%B']
        ...
        if msg_cur_line == 1:
            ...
            if 'Revert' not in msg_line_text:
                ...
                if module_name[-1] != ':':
                    ...
                    CPrint('ERR_MSG', "Other common module names include layers, build, cmake, tests, docs, scirpts, stateless, gpu, syncval, practices, etc.")
                    CPrint('CONTENT', "     '" + msg_line_text + "'\n")
                    retval = 1
                else:
